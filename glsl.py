...
def select_shader(emotion):
    if emotion == 'Sad':
        ...
        float sHash11(float p) {
            return mix(hash11(p), hash11(p + 1.0), smoothstep(0.0, 1.0, p));
            }
        ...
        float Eye(vec2 p, float pupil, vec2 lpos){
            ...
            //noise frequency for ratial coords
            vec2 freq = vec2(30.,1.5);
            
            //radial noise
            float f = pow((cnoise(pr*freq,freq.x)+1.0)/4.0, 0.5);
            f -= pow((cnoise(freq*pr*vec2(2.0,3.0)+9.0,2.0*freq.x)+1.0)/2.0-0.5, 2.0);
            
            //shading
            float shade = dot(p,lpos);
            
            //lightening shade of iris
            f -= .7 * shade;
            ...
            f = mix(f, .25,smoothstep(.5,1.,pr.y+2.));
            
            //mix in scleara
            f = mix(f, 1.0 - 0.2 * dot(p, p) + 0.75 * shade, smoothstep(0.7, 0.85, pr.y));
            
            //highlight
            //f = mix(1.0, f, clamp((length(p - lpos) - 0.15)/0.025, 0.0, 1.0));
        ...
        void main(void){
            ...
            // Correct the division operation and ensure all variables are properly declared and used
            vec2 p = 5.* (texel_coord - resolution.xy / 2.0) / resolution.y;

            // Correct the declaration of t
            float t = 1.0 + 0.05 * sHash11(time * 1.5);

            // Ensure position is declared and passed correctly
            vec4 output_color = vec4(vec3(Eye(p, t, position*t)), 1.0);
...
def set_uniforms_for_shader(emotion,shader_program):
    if emotion == 'Sad':
        ...
        shader_program['position'] = (float(window.width/4),float(window.height/4))
